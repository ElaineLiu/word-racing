...
### #014 - 3D 赛车倒计时期间相机未对准起点
**发现时间**: 2026-06-10
**问题类型**: UX问题
**严重程度**: 高

**问题描述**:
在主界面点击 RACE 开始 3D 比赛时，倒计时期间只能看到绿色背景和倒计时数字，看不到赛道和赛车。倒计时结束后比赛才正常显示。

**原因分析**:
1. CameraController 使用 `lerp` 平滑跟随赛车，默认 lerp factor 为 0.1。
2. 相机初始位置不在起点，需要多帧才能移动到正确位置。
3. RaceSession3D 构造函数创建了 CameraController，但没有立即定位相机。
4. 倒计时只有 4 秒（240帧），相机还在移动过程中倒计时就结束了。

**解决方案**:
1. 在 CameraController 中添加 `snapTo()` 方法，直接将相机移动到目标位置（不使用 lerp）。
2. 在 RaceSession3D 构造函数中创建 CameraController 后立即调用 `snapTo()`。
3. 在 test-3d.html 的初始化和重置流程中也调用 `snapTo()`。

**预防措施**:
1. ✅ 所有需要立即定位相机的场景（初始化、重置、传送）必须使用 `snapTo()`，不能依赖 `update()` 的 lerp。
2. ✅ 测试倒计时/过场动画时，必须验证视觉效果是否符合预期，不能只验证逻辑状态。
3. ✅ 相机初始化测试应该包含初始位置验证，确保第一帧就能看到目标。

**相关文件**:
- `3d/controllers/camera-controller.js` - 添加 snapTo() 方法
- `3d/runtime/race-session-3d.js` - 构造函数中调用 snapTo()
- `test-3d.html` - 初始化和重置时调用 snapTo()
- `tests/3d/camera-controller.test.js` - 新增 snapTo() 测试

---

### #013 - 3D 闭环赛道起终点区域生成杂乱护栏/路缘块
**发现时间**: 2026-06-10
**问题类型**: UI/UX问题
**严重程度**: 中

**问题描述**:
3D 上海赛道起终点线附近出现一堆杂乱的白色方块，视觉上像起点生成错误。实际主要是闭环最后一段与起点交接时，护栏和路缘石采样点落入起终点区域。

**原因分析**:
1. `TrackBuilder.addBarriers()` 和 `addKerbs()` 沿完整闭环采样，没有为起终点线预留清场区域。
2. 闭环曲线最后一段回到 `centerPoints[0]`，靠近起点的采样点会在棋盘线前后继续生成 block。
3. 原测试只验证“生成了护栏/路缘”，没有验证真实赛道起终点区域是否保持干净。

**解决方案**:
1. 在护栏和路缘生成时跳过距离起点小于 `trackWidth` 的采样点。
2. 为真实 `shanghai-3d` 配置补充回归测试：起点 `trackWidth` 范围内不能有 `barrier` 或 `kerb` mesh。

**预防措施**:
1. ✅ 3D 几何测试不能只验证对象存在，还要验证关键区域没有不该出现的对象。
2. ✅ 闭环赛道必须单独测试起终点交接处，尤其是装饰物、护栏、路缘石等沿路径采样生成的对象。
3. ✅ 回归测试优先使用真实赛道配置，避免简单矩形测试漏掉真实曲线问题。

**相关文件**:
- `3d/rendering/track-builder.js`
- `tests/3d/track-builder.test.js`
