...
**创建日期**: 2026-06-07
**最后更新**: 2026-06-07（Epic 1 启动）
**状态**: 🚧 实施中（Epic 0 ✅ / Epic 1 🚧）
...
- [ ] 构造函数接受可选 `{ rendererFactory }` 注入项（便于 jsdom 单测使用 fake renderer）
- [ ] 初始化 `THREE.Scene`、`THREE.PerspectiveCamera`、`THREE.WebGLRenderer`
- [ ] WebGLRenderer 设置 `alpha: true`（透明背景方便HUD叠加）
- [ ] 添加环境光 + 方向光
- [ ] 提供 `render()` 方法触发渲染
- [ ] 提供 `update(deltaTime)` 方法更新动画（占位，未来扩展）
- [ ] 提供 `resize(width, height)` 方法处理画布大小变化（由外部显式触发，不自动绑定 window.resize）
...
- [ ] 构造函数接受 `THREE.Camera` 实例 + 可选 `{ positionLerpFactor = 0.1 }`
- [ ] 支持两种模式：`'chase'`（追尾）、`'cockpit'`（驾驶舱），默认 `'chase'`
- [ ] 追尾视角：赛车后上方（距离50单位，高20单位）
- [ ] 驾驶视角：驾驶舱内（高3单位）
- [ ] `update(carLike)` 接受任意 `{ x, y, angle }` 对象（2D y → 3D z），不强依赖 Car3D 类
- [ ] 使用 lerp 平滑插值（`positionLerpFactor=0.1`），单帧不瞬移
- [ ] `toggleMode()` 在 chase / cockpit 之间切换
- [ ] `setMode(mode)` 设置模式，无效模式抛错
