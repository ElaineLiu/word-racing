...
**Acceptance Criteria**:
- [ ] `test-3d.html` 创建 `Scene3D` 实例（注入自定义 rendererFactory 绑定到 #three-canvas）
- [ ] 场景包含一个绿色地面 Plane（10000×10000）作视觉参考
- [ ] 场景包含一个红色 Box 占位赛车（4×2×8），标注"TEMP for Epic 1 only"
- [ ] 渲染循环 60fps 运行，FPS 显示在调试面板
- [ ] 方向键 ↑↓←→ 驱动占位赛车，CameraController 平滑跟随
- [ ] 按 C 键切换 chase / cockpit 视角，调试面板显示当前模式
- [ ] `window.resize` 调用 `scene3d.resize(w, h)`，画面不变形
- [ ] 调试面板显示：FPS、Three.js 版本、camera mode、Box 位置、Box 角度、mock mastered/coins
